...
<html lang="en">
  ...
  <ul class="dropdown-menu">
    ...
    <div class="container">
      ...
      <div class="jumbotron">
        <h2>กระดูกหัก</h2>
         <p>
            โปรดเลือกลักษะอาการย่อย ของผู้ป่วยตามหัวข้อด้านล่างนี้<br>
            <font color="red">***ในกรณีที่ไม่มีอาการย่อย ในกดถัดไปเพื่อทราบวิธีการปฐมพยาบาลได้เลย</font>
        </p>
      </div>
 ...
 <div class="container">
        ...
        $case_name = $_GET['case_name'];
          ...
          $sql = "SELECT case_sym_flag FROM sym_cases_level1 where case_name='".$case_name."'";
          ...
          // output data of each row
            ...
            while($row = $result->fetch_assoc()) {
             $lv1_case_sym_flag = $row['case_sym_flag'];
          }
...
<div class="jumbotron">
        ...
        <form action="sym_bone_problem_result.php" method="GET">

        <div class="radio">
        
        <?php

           $sql = "SELECT case_name,case_info FROM sym_cases_level2 where level1_sym_flag='".$lv1_case_sym_flag."'";
          $result = $connect_db->query($sql);

          if ($result->num_rows > 0) {
          // output data of each row
         while($row = $result->fetch_assoc()) {
            echo '<label><input type="radio" name="lv2_case_name" value="'.$row["case_name"].'" required>' . $row["case_info"].'</label>' .'<br>';
          }
          } else {
            echo "0 results";
          }
    ...
    <button class="btn btn-default" ><a href="sym_bone_problem.php">ย้อนกลับ</a></button> 
